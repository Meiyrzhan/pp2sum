...
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pygame.image.load("Coin.png")  # Assume you have a coin image
        self.rect = self.image.get_rect()
...
while True:
    ...
    if random.randint(1, 100) < 2:  # 2% chance each frame to spawn a coin
        new_coin = Coin()
        coins.add(new_coin)
    ...
    for coin in collected_coins:
        COINS_COLLECTED += 1
        print(f"Collected a coin! Total coins: {COINS_COLLECTED}")
